// ETL do navegador — gerado de etl.mjs (não editar à mão)
window.WTA_ETL = function(buf, hoje, DRILL_K){
  DRILL_K = DRILL_K || 12;
  const HOJE = (hoje instanceof Date && !isNaN(hoje)) ? hoje : new Date();
...
if (!sheet) { console.error('❌ Aba "2022-2026" não encontrada em ' + "a planilha" + '. Abas: ' + wb.SheetNames.join(', ')); throw new Error("aba 2022-2026 ausente"); }
